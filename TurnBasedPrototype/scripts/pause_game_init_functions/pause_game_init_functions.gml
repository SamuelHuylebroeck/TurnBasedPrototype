//@description ??
function toggle_pause_game_bring_up_menu(){
	toggle_pause_game()
	global.in_game_pause_menu_active = not global.in_game_pause_menu_active
	with(obj_next_turn_button){
		visible = not global.in_game_pause_menu_active
	}
	with(obj_pause_menu)
	{
		//Create or destroy the current page
		if global.in_game_pause_menu_active 
		{
			create_pause_page_visualization(pages[current_menu_page_index])
		}else{
			save_config_to_file(pages[2],pages[3], pages[4], pages[5])
...
function toggle_pause_game(){
	global.map_running = not global.map_running
}


#region pause menu settings pages
function init_pause_game_pages()
{
	var config_map = load_config_from_file()
	
	var ds_menu_main = create_menu_page([
		["Resume",		menu_element_types.script_runner,		toggle_pause_game_bring_up_menu],
		["Settings" ,	menu_element_types.page_transfer,		pause_menu_page.settings],
		["Main Menu",	menu_element_types.script_runner,		exit_to_main_menu],
		["Exit Game",	menu_element_types.script_runner,		exit_game]
	])
	var ds_menu_settings = create_menu_page([
		["Audio",		menu_element_types.page_transfer,  pause_menu_page.audio],
		["Video",		menu_element_types.page_transfer,  pause_menu_page.video],
		//["Controls",	menu_element_types.page_transfer,  pause_menu_page.controls],
		["Game",		menu_element_types.page_transfer,  pause_menu_page.game],
		["Back",		menu_element_types.page_transfer,  pause_menu_page.main],
	...
	var ds_menu_audio = create_menu_page([
		...
		["Back",	menu_element_types.page_transfer,	pause_menu_page.settings],
	])
	...
	var pages = [ds_menu_main, ds_menu_settings, ds_menu_audio, ds_menu_video, ds_menu_controls, ds_menu_game]
	return pages
...
{
	var	arg, i=0;
	var arg_count = array_length(arguments)
	repeat(arg_count)
	{
		arg[i] = arguments[i]
		i++;
	...
	var ds_grid_id = ds_grid_create(5, arg_count)
	
	i=0
	repeat(arg_count){
		var array = arg[i]
		var array_len = array_length(arg[i])
		var xx = 0;
		repeat(array_len)
		{
